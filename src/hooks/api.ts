...
export function useAPIFetch<T>(path?: string, init?: Omit<RequestInit, 'signal'>): [T | undefined | Error, () => Promise<void>] {
    ...
    }, [path, apiRoot, token, init, uri, dispatch]);
    return [data, refetch];
}
...
export function useAPIObjectState<T extends APIObject>(path?: string, initialState?: T): [T | undefined | Error, (next: T) => Promise<T>] {
    ...
    const update = useCallback(async (next: T) => {
        ...
        /*
        if (state && !(state instanceof Error)) {
            return fetch(new URL(`${getObjectRoot(state.modelType)}/${state.id}`, apiRoot).toString(), { method: next.id ? 'PUT' : 'POST', body: JSON.stringify(next), headers: { authorization: `Bearer ${token}` } }).then((response) => {
                response.json().then((data) => {
                    if (response.ok) {
                        setState(data);
                    } else {
                        // setState(new Error(response.statusText));
                        console.log(data);
                    }
                }).catch(e => {
                    //
    ...
    console.log(`useAPIObjectState`, state, path, initialState);
    return [state, update];
}